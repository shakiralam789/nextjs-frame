 ...
 */
export default function Breadcrumb({ items, className = "", Icon }) {
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    if (pathname) {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        const hasIdAfterDynamic =
          isDynamicSegment && nextSegment && /^\d+$/.test(nextSegment);
  ...
  return (
    <nav
      className={`flex items-center flex-wrap justify-between font-14 mb-4 2xl:mb-6 ${className}`}
